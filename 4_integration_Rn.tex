...
\lemma{3.14L)}{
    A vectorfield $f:X\to \Rn$ is conservative $\iff$ for each closed parameterized curve $\gamma$ contained in $X$ it holds that $\int_\gamma f(s)d\vec{s}=0$.
}
...
\definition{3.16L (path-connected space, potential)}{
    \begin{enumerate}[label=(\arabic*)]
...
\other{Example, polar coordinates (L)}{
...
\definition{3.39L Jordan curve, orientation }{
...
\definition{3.40 \textit{reguläres Gebiet} (L)}{
...
\theorem{4.6.3 (3.41L) Green's formula}{
