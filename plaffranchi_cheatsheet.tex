...
\begin{multicols*}{3}
    ...
    \definition{3.4.13.}{Let $X \subset \Rn$ be an open set and let $f: X \rightarrow \Rm$ be a function. Let $v \in \Rn$ be a non-zero vector and $x_{0} \in X$. We say that $f$ has \textbf{directional derivative $w \in \Rm$ in the direction $v$}, if the function $g$ defined on the set
        $$
            I=\left\{t \in \mathbf{R}: x_{0}+t v \in X\right\}
        $$
        by
        $$
    ...
    \prop{3.5.4 (Mixed derivatives commute)}{
        $k \geqslant 2$. Let $X \subset \mathbf{R}^{n}$ be open and let $f: X \rightarrow \mathbf{R}^{m}$ be a function of class $C^{k}$. Then the partial derivatives of order $k$ are independent of the order in which the partial derivatives are taken: for any variables $x$ and $y$, we have
        $
    ...
    \definition{3.5.9 (Hessian).}{Let $X \subset \mathbf{R}^{n}$ be open and $f: X \rightarrow \mathbf{R}$ a $C^{2}$ function. For $x \in X$, the \textbf{Hessian matrix} of $f$ at $x$ is the symmetric square matrix
    $$
